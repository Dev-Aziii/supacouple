...
BEGIN
  ...
  SELECT * INTO v_receiver_profile FROM public.profiles WHERE id = p_user_id;
  ...
  WHERE status = 'pending'
    AND id <> v_invite.id
    AND (sender_id IN (v_invite.sender_id, p_user_id) OR receiver_id IN (v_invite.sender_id, p_user_id));
...
BEGIN
  ...
  FROM public.couples
  WHERE (created_by = p_user_id OR created_by = v_partner_id)
    AND status = 'active'
  LIMIT 1;
